...
Supervised learning involves training a machine with labeled data. A label is normally explicit metadata that describes an source in a info set. This label can be in the type of some identifier, classification or judgment that the device learning algorithm may use to characterize the info it is processing.
